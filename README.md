...
H6-C4D/C5 复用同一 MCP 服务提供 `app://rikka/observation/v1/latest` 与 `submit_rikka_decision_v1`：前者读取由同帧 JoinedState 纯映射的不可变观测，后者按配对 provenance/UID 验证 Rikka 决策，并以原配证据 dry-run 复用 H6-A 安全门及可用时的 H6-B resolver。Rikka 是唯一高层策略大脑；C5 在 coordinator/transport 前硬停止，production 坐标校准留待 C6，真实动作不可达且 `ACTION_EXECUTED=0`。
